...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        return boar_brawl(player_score, opponent_score) # score when roll 0 dices
    
    return roll_dice(num_rolls, dice) # score when roll > 0 dices
    # END PROBLEM 3
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    base_value = strategy(0, 0)
    player = 0
    
    while player < goal:
        opponent = 0
        while opponent < goal:
            if strategy(player, opponent) != base_value:
                return False
            opponent += 1  # one player score is corresponding to 100 opponent scores
        player += 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    >>> max_scoring_num_roll) == int, "num_rolls must be an integer."
    assert num_rolls > 0,s(dice)
    1
    """
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    ...
    value_list = []
    while count < 11:
        curr = make_averaged(roll_dice, times_called)(count, dice) 
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    final = boar_brawl(score, opponent_score)
    if final >= threshold:
        return 0
    else:
        return num_rolls
