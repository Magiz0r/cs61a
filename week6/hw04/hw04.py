def shuffle(s):
    ...
    #[0, 1, 2, 3, 4, 5]
    empty_list = []
    list_length = len(s) // 2 # using floor division for integer
    
    s0 = s[0:list_length]
    s1 = s[list_length:]
    
    for i in range(list_length):
        empty_list = empty_list + [s0[i]] + [s1[i]]
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_torque = left_length * left_mass
        right_torque = right_length * right_mass
...
def max_path_sum(t):
    ...
    # label + label + label recursion, then compare each other
    max_value = 0
    if is_leaf(t):
        return label(t)
    ...
    for b in branches(t):
        if is_tree(b):
            if label(t) + max_path_sum(b) > max_value:
                max_value = label(t) + max_path_sum(b)
